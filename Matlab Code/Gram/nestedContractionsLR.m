...
function [G] = nestedContractionsLR(X, Y)
% This subroutine computes the sequence of contractions between the TT 
% tensor $X$ and the subparts of the TT tensor $Y$ from Left to Right
...
G = cell(N, 1);
% Note that G{N} will be the inner product between X and Y

% forming G_n = contractions between cores (1, \ldots, n) of X and Y 
G{1} = X{1}' * Y{1};
for i = 2 : N
    temp = G{i - 1} * V2H(Y{i}, I(i));
    G{i} = X{i}' * H2V(temp, I(i));
end
